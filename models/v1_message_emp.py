...
from utils.utils import get_indexes
...
class EdgeMessagePassingLayer(torch.nn.Module):
    """
    ...
    """
    def __init__(self, emb_dim, hidden_dim, num_relations,input_layer=False, num_bases= None):
        ...
        # self.mlp = torch.nn.Sequential(torch.nn.Linear(hidden_dim, hidden_dim), torch.nn.BatchNorm1d(hidden_dim), torch.nn.ReLU(), torch.nn.Linear(hidden_dim, hidden_dim))
        
        self.conv = RGCNConv(emb_dim, hidden_dim, num_relations,self.num_bases)
        self.bn = BatchNorm(hidden_dim)

    def forward(self, x,edge_index, edge_type):
        # x,edge_index, edge_type = relation_line_G.x, relation_line_G.edge_index, relation_line_G.edge_type
        # print(x.size())
        x = F.leaky_relu(self.bn(self.conv(x, edge_index, edge_type)))
        # out = self.mlp(x)
        return x
...
class CustomRGCNConv(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.fc = torch.nn.Linear(self.out_channels, self.in_channels)  # 512, 21 
    ...
    def forward(self, x: Union[OptTensor, Tuple[OptTensor, Tensor]],
                edge_index: Adj, edge_type: OptTensor = None, edge_attr=None):
        r"""
        Args:
        ...
        if self.num_blocks is not None:  # Block-diagonal-decomposition =====
            ...
            for i in range(self.num_relations):
                ...
                h = self.propagate(tmp, x=x_l, edge_type_ptr=None, size=size,edge_attr = edge_attr)
            ...
            if self._WITH_PYG_LIB and isinstance(edge_index, Tensor):
                ...
                out = self.propagate(x=x_l,edge_index = edge_index, 
                                     edge_type_ptr=edge_type_ptr, size=size,edge_attr = edge_attr)
            else:
                for i in range(self.num_relations):
                    tmp = masked_edge_index(edge_index, edge_type == i)
                    ...
                    if x_l.dtype == torch.long:
                        ...
                        kk = edge_type.clone().detach()
                        mask_idx = kk == i
                        h = self.propagate(tmp, x=x_l, edge_type_ptr=None,
                                           size=size,masked_edge_attr = edge_attr[mask_idx])
                        out = out + (h @ weight[i])
    ...
    def message(self, x_i,x_j,edge_index_i, edge_index_j,edge_type_ptr, masked_edge_attr):
        '''
           m(j,i,r) = num_edges * hidden dim
           x_j = x_j + linear(m(j,i,r))
           x_j + self.fc(edge_attr[index]) 
           key: find the indexes corresponding edge_attr
        '''
        if edge_type_ptr is not None:
            return segment_matmul(x_j, edge_type_ptr, self.weight)        
        masked_edge_attr = self.fc(masked_edge_attr)
        updated = x_j + masked_edge_attr
        return updated
